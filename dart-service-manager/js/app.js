...
function adaptive_testimonials(w, h) {
	...
	if(w < 767) {
		if(!testimonials__btn.hasClass('small-screen')) {
			testimonials__btn.addClass('small-screen').appendTo(testimonials__row);
		}
	}
	else {
		if (testimonials__btn.hasClass('small-screen')) {
			testimonials__btn.removeClass('small-screen').appendTo($('.testimonials__column').eq(0));
		}
	}
}
